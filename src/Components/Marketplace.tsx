...
const Marketplace: React.FC = () => {
  ...
  const createEmailLink = (coin: any) => {
    ...
    const body = `Dear Rosland Capital,\n\nI am interested in purchasing the following coin:\n\n- Coin Name: ${coin.name}\n- Price: $${coin.price}\n- Year: ${coin.year}\n- Material: ${coin.material}\n- Condition: ${coin.condition}\n- Era: ${coin.era}\n\nPlease contact me with further details regarding purchase.`;
    return `mailto:roslandcapitaluk@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800">
      <Navbar />
      {/* Header */}
      <div className="bg-gradient-to-r from-gray-900 to-gray-800 border-b border-yellow-500/20 py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <h1 className="text-4xl md:text-5xl font-bold text-yellow-300 mb-4">Coin Marketplace</h1>
            ...
            <div className="mt-8 flex items-center justify-center gap-4">
              <div className="bg-yellow-500/20 text-yellow-300 px-4 py-2 rounded-lg border border-yellow-500/30">
                <span className="font-semibold">{coins.length}</span> Coins Available
              </div>
              <div className="bg-yellow-500/20 text-yellow-300 px-4 py-2 rounded-lg border border-yellow-500/30">
                <span className="font-semibold">Rosland Capital Certified</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-1/4">
            <div className="bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-xl border border-yellow-500/20 p-6 sticky top-8">
              ...
              {/* Search Box */}
              <div className="mb-6">
                <label className="block text-yellow-300 font-semibold mb-2">Search Coins</label>
              ...
              {/* Price Range */}
              <div className="mb-6">
                <label className="block text-yellow-300 font-semibold mb-2">Price Range</label>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <input
                      type="number"
                      placeholder="Min"
                      className="w-full px-4 py-2 bg-gray-800 border border-yellow-500/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 text-gray-200 placeholder-gray-500"
                      value={minPrice}
              ...
              {/* Era Filter */}
              <div className="mb-6">
                <label className="block text-yellow-300 font-semibold mb-2">Historical Era</label>
              ...
              {/* Rarity Filter */}
              <div className="mb-6">
                <label className="block text-yellow-300 font-semibold mb-2">Rarity Level</label>
              ...
              {/* Sort By */}
              <div className="mb-6">
                <label className="block text-yellow-300 font-semibold mb-2">Sort By</label>
                <select
          ...
          <div className="lg:w-3/4">
            {/* Market Stats Widget */}
            <div className="mb-8 bg-gradient-to-r from-gray-800 to-gray-900 border border-yellow-500/20 text-white rounded-2xl shadow-xl p-6">
              <div className="flex flex-col md:flex-row justify-between items-center">
                <div>
                  <h3 className="text-2xl font-bold text-yellow-300 mb-2">Market Insights</h3>
                  <p className="text-gray-400">Real-time coin market data</p>
                </div>
                <div className="flex items-center space-x-6 mt-4 md:mt-0">
                  <div className="text-center">
                    <div className="text-3xl font-bold text-yellow-300">${Math.min(...coins.map(c => c.price)).toLocaleString()}</div>
                    <div className="text-sm text-gray-400">Lowest Price</div>
                  </div>
                  <div className="text-center">
                    <div className="text-3xl font-bold text-yellow-300">${Math.max(...coins.map(c => c.price)).toLocaleString()}</div>
                    <div className="text-sm text-gray-400">Highest Price</div>
                  </div>
                  <div className="text-center">
                    <div className="text-3xl font-bold text-yellow-300">${Math.round(coins.reduce((sum, coin) => sum + coin.price, 0) / coins.length)}</div>
              ...
              <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-yellow-500/10 p-4 rounded-lg text-center border border-yellow-500/20">
                  ...
                  <div className="text-xl font-bold text-yellow-300">{coins.filter(c => c.material === 'Gold').length}</div>
                </div>
                <div className="bg-yellow-500/10 p-4 rounded-lg text-center border border-yellow-500/20">
                  <div className="flex items-center justify-center mb-2">
                    <DollarSign className="w-6 h-6 text-yellow-400" />
                  </div>
                  <div className="text-sm text-gray-300">Silver Coins</div>
                  <div className="text-xl font-bold text-yellow-300">{coins.filter(c => c.material === 'Silver').length}</div>
                </div>
                <div className="bg-yellow-500/10 p-4 rounded-lg text-center border border-yellow-500/20">
                  <div className="flex items-center justify-center mb-2">
                  ...
                  <div className="text-xl font-bold text-yellow-300">{coins.filter(c => c.featured).length}</div>
                </div>
                <div className="bg-yellow-500/10 p-4 rounded-lg text-center border border-yellow-500/20">
                  <div className="flex items-center justify-center mb-2">
                  ...
                  <div className="text-xl font-bold text-yellow-300">{coins.length}</div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredCoins.map((coin) => (
                <div 
                  key={coin.id} 
                  className={`bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-yellow-500/20 overflow-hidden hover:shadow-xl transition-all duration-300 relative ${
                    coin.featured ? 'ring-2 ring-yellow-500' : ''
                  }`}
                >
                  {coin.featured && (
                    <div className="bg-yellow-500 text-gray-900 text-xs font-bold px-3 py-1 absolute top-3 left-3 rounded-full z-10">
                      FEATURED
                    </div>
                  )}
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        ...
                        <div className="flex items-center mt-1">
                          ...
                          <span className="text-sm text-gray-400">{coin.year}</span>
                        </div>
                      </div>
                      <button
                      ...
                      >
                        <Heart 
                          className={`w-6 h-6 ${favorites.includes(coin.id) ? 'fill-current text-red-500' : ''}`}
                        />
                      </button>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-6">
                      ...
                      <div className="bg-gray-700/50 p-3 rounded-lg">
                        ...
                        <div className="text-lg font-semibold text-yellow-300">{coin.material}</div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between mb-6">
                      <div className="flex items-center">
                        ...
                        <span className="font-semibold text-gray-300">{coin.condition}</span>
                      </div>
                      <div className={`px-3 py-1 rounded-full text-sm font-semibold ${
                        coin.rarity === 'Very Rare' ? 'bg-yellow-500/20 text-yellow-300 border border-yellow-500/30' :
                        coin.rarity === 'Rare' ? 'bg-yellow-500/20 text-yellow-300 border border-yellow-500/30' :
                    ...
                    <div className="space-y-3">
                      <a
                        ...
                        className="block w-full bg-yellow-500 text-gray-900 py-3 rounded-lg font-semibold hover:bg-yellow-400 transition-all duration-300 text-center shadow-lg hover:shadow-xl"
                      >
                        Express Interest
                      </a>
      ...
      {/* Bottom CTA */}
      <div className="bg-gradient-to-r from-gray-900 to-gray-800 border-t border-yellow-500/20 text-gray-200 py-12 mt-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-3xl font-bold text-yellow-300 mb-4">Looking for a Specific Coin?</h2>
          <p className="text-xl text-gray-400 max-w-3xl mx-auto mb-8">
            Our experts can help you find rare and specific coins not listed in our marketplace.
          </p>
          <a
            href="mailto:roslandcapitaluk@gmail.com?subject=Custom%20Coin%20Search%20Request"
